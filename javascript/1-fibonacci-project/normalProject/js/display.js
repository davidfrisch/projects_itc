...
/**
 * Display the fibonacci value depending of the input field
 */
...
/**
 *  Add an HTML <p> tag to the history list
 *
 * @param {number} number 
 * @param {number} result 
...
/**
 * Display the history of the last fibonacci calculations 
 *
 * @param {function} sortAlgorithm 
 */
function displayHistoryFibonacci(sortAlgorithm = sortByDateDesc) {
...
/**
 * 
 * @returns the history of computed fibonacci from the server
 * e.g :
 * {{number:3; result:5, createdDate:1635604694742}, 
 *  {number:1; result:1, createdDate:1635614694442}
 * }
 */
function getHistoryFibonacci() {
    const FIBONACCI_HISTORY = FIBONACCI_SERVER + 'getFibonacciResults/'
    return new Promise((resolve, reject) => {
        fetch(FIBONACCI_HISTORY)
            .then((response) => {
                if (response.ok) {
                    response.json()
                        .then((data) => {
                    ...
                    reject('Something went wrong with getFibonacciResult')
                }
            })
            .catch((err) => {
                reject(err)
